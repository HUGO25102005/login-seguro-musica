...
| **A03:2021-Injection (XSS)** | Content Security Policy (CSP) y React Escape. | `middleware.ts` / Next.js Core |
| **A04:2021-Insecure Design** | Arquitectura SSR con cookies HttpOnly. | `utils/supabase/server.ts` |
| **A07:2021-Identification & Auth Failures** | Rate Limiting y Google OAuth 2.0. | `middleware.ts` / Supabase Auth |
...
Nuestra política `default-src 'self'` es restrictiva. Impide que la aplicación cargue scripts de dominios no autorizados, neutralizando ataques XSS donde el atacante intenta inyectar un script remoto para robar el token de sesión.
